...
test("generated invoices: authenticated actions, relevant periods, empty outcomes and review",{timeout:180000},async(t)=>{
 ...
 assert.ok(actionId("generateInvoice"));
 ...
 try {
  ...
  const protectedResponse=await request("/invoices/generate",{},false);assert.ok(protectedResponse.headers.get("location")?.includes("/login")||(await protectedResponse.text()).includes("NEXT_REDIRECT;replace;/login;"));
  ...
  const formPage=await page("/invoices/generate");assert.match(formPage,/Include eligible billing activity through this date/);
  ...
  const payload={customer_id:customer,issue_date:"2025-09-20",as_of_date:"2025-09-20",notes:"Generated fixture",terms:"Review first"};
  async function generate(body,requestId=randomUUID(),auth=true){const form=new FormData();form.set("payload",JSON.stringify(body));form.set("request_id",requestId);const r=await action("generateInvoice","/invoices/generate",form,auth);return {text:await r.text(),response:r,requestId};}
