...
test("invoice Ready lifecycle: fresh replay, edits, history and source regressions", { timeout: 60000 }, async (t) => {
  const database = `nexa_ready_test_${randomBytes(6).toString("hex")}`;
  let created = false;
  try {
    ...
    const target = "20260916120000_invoice_ready_status.sql";
    ...
    for (const file of ["customers_privileges.sql", "customer_billing_agreements.sql", "billing_same_day_end.sql",
      "change_customer_billing_terms.sql", "time_entries.sql", "retainer_usage.sql", "running_timers.sql", "invoices.sql", "create_manual_invoice.sql", "generate_customer_invoice.sql", "invoice_composer.sql", "edit_invoice.sql", "invoice_ready.sql"]) {
