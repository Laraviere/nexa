...
test('report renders full-set aggregates, canonical links, mobile cards and authoritative status labels',()=>{
 ...
 assert.match(html,/name="from"[^>]*value=""/);assert.match(html,/name="to"[^>]*value=""/);assert.match(html,/nexa-table-frame/);assert.match(html,/About these totals/);
 assert.match(html,/method="get"/);assert.match(html,/name="workflow"/);assert.match(html,/aria-label="Invoice cards"/);assert.match(html,/class="nexa-table"/);assert.match(html,/nexa-records/);assert.match(html,/href="\/invoices\/12345678/);assert.match(html,/page=2/);
