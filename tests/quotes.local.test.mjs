...
test('Quotes: authenticated routes/actions, conversion, PDFs and backdated invoice UI',{timeout:180000},async(t)=>{
 ...
 let customer,userId,invoiceId,quoteId;
 ...
 try {
  ...
  const quotePdf=await request(route+'/pdf');assert.equal(quotePdf.status,200);const quoteText=(await inspectPdf(Buffer.from(await quotePdf.arrayBuffer()))).text;assert.match(quoteText,/QUOTE/);assert.doesNotMatch(quoteText,/INVOICE|DUE DATE/);
  const invoicePdf=await request(`/invoices/${invoiceId}/pdf`);assert.equal(invoicePdf.status,200);assert.match((await inspectPdf(Buffer.from(await invoicePdf.arrayBuffer()))).text,/September 1, 2026/);
