...
function harness() {
  ...
  const overrides={"@/lib/customers/server":{customerClient:async()=>client},"next/cache":{revalidatePath:()=>{}}};
  const cache=new Map();
  function load(name){
  ...
  return {model:load("@/lib/invoices/model"),actions:load("@/actions/invoices"),calls,setError:e=>error=e,setThrows:()=>throws=true};
}
