...
export function CustomerBilling({ customerId, agreements, today, usage, paymentTerms }: { customerId: string; agreements: BillingAgreement[]; today: string; usage?: RetainerUsageResult; paymentTerms?: string }) {
  ...
  return (
    <Surface aria-labelledby="billing-heading" className="mt-6">
      <SectionHeading id="billing-heading">Billing</SectionHeading>
      {paymentTerms&&<p className="mt-3 text-sm text-secondary">Default payment terms: <span className="font-medium text-ink">{paymentTerms}</span></p>}
      <div className="mt-5 flex flex-wrap items-start justify-between gap-4">
        ...
        {current && canChangeBillingTerms(current, agreements, today) && <ButtonLink href={`/customers/${customerId}/billing/${current.id}/change`}
          variant="primary">Change billing terms</ButtonLink>}
        <ButtonLink href={current ? `/customers/${customerId}/billing/${current.id}/end` : `/customers/${customerId}/billing/new`}
        ...
        {agreements.length === 0 ? <p className="mt-4 text-sm text-slate-600">No billing agreements yet. This customer can remain without a retainer.</p> : (
          <ul className="mt-4 divide-y divide-slate-200">
            {agreements.map((agreement) => {
              ...
              return <li key={agreement.id} className="py-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className={`rounded px-2 py-1 text-xs font-medium ${status === "Current" ? "bg-emerald-50 text-emerald-700" : status === "Future" ? "bg-cyan-50 text-cyan-800" : "bg-slate-100 text-slate-600"}`}>
                ...
                {status === "Future" && canChangeBillingTerms(agreement, agreements, today) && <ButtonLink
                  href={`/customers/${customerId}/billing/${agreement.id}/change`}
                  className="mt-4">Change billing terms</ButtonLink>}
              </li>;
            })}
          </ul>
        )}
