...
export default async function EditInvoicePage({params}:{params:Promise<{id:string}>}) {
 ...
 return <PageContainer width="workspace"><div className="nexa-form-page"><Link href={`/invoices/${invoice.id}`} className="text-sm font-medium text-cyan-700">← Invoice #{invoice.invoice_number}</Link><PageHeader title="Edit Invoice" status={null} actions={null}/><p className="mb-6 mt-2 text-slate-500">Invoice #{invoice.invoice_number} · Changes save together.</p><CreateInvoiceForm customers={options} today={today} initialRequestId={randomUUID()} owner={owner} editing={editing}/></div></PageContainer>;
}
