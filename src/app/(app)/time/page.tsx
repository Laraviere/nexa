...
import { getRecentTime, getTimeCustomers } from "@/lib/time/server";
...
export default async function TimePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ entries, hasNext }, { timer, observedAt }, customers] = await Promise.all([getRecentTime(page), getTimer(), getTimeCustomers()]);
  return <>
    ...
    <TimerPanel timer={timer} customers={customers} today={businessDate()} serverNow={observedAt} />
