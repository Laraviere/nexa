-- Ready is an editable workflow state, not a delivery event. No existing row
-- or timestamp is rewritten. Preserve recorded sent_at on subsequent transitions.
alter table public.invoices drop constraint invoices_status_check;
alter table public.invoices add constraint invoices_status_check check (status in ('draft','ready','sent','void'));
alter table public.invoices drop constraint invoices_check2;
alter table public.invoices add constraint invoices_status_timestamp_check check (status <> 'sent' or sent_at is not null);
...
begin
  ...
  if old.status='void' and (to_jsonb(new)-'updated_at') is distinct from (to_jsonb(old)-'updated_at') then
    raise exception 'Void invoices are historical and cannot be edited.';
  end if;
  if new.status is distinct from old.status then
    if old.status = 'void' or new.status not in ('draft','ready','sent','void') then raise exception 'Invalid invoice lifecycle transition.'; end if;
    if new.status in ('ready','sent') then
      if not exists (select 1 from public.invoice_items where invoice_id = new.id and superseded_at is null) then raise exception 'An invoice needs line items.'; end if;
      if exists (
        select 1 from public.invoice_items i where i.invoice_id = new.id and i.superseded_at is null and i.billed_minutes is not null
      ...
      ) then raise exception 'Time line minutes must exactly match source allocations before marking Ready or Sent.'; end if;
      if new.status='sent' then new.sent_at:=coalesce(old.sent_at,clock_timestamp());end if;
...
begin
  ...
  -- REPEATABLE READ (which must retry rather than accept a stale snapshot).
  ...
  if tg_op='UPDATE' and new.superseded_at is not null then
    if p.status='void' or (to_jsonb(new)-array['superseded_at','updated_at','amount']) is distinct from (to_jsonb(old)-array['superseded_at','updated_at','amount']) then
  ...
  if p.status = 'void' then raise exception 'Void invoice lines cannot be edited.'; end if;
...
create or replace function public.update_composed_invoice(
...
begin
 ...
 if found then
  ...
  update public.invoices set issue_date=p_issue_date,notes=p_notes,terms=p_terms where id=rid;
...
comment on table public.invoices is 'Editable Draft/Ready/Sent invoices; Void preserves history. Ready is internal readiness, not sending or a financial lock. Sent is a recorded status, not proof of delivery without a delivery system. Status transitions preserve existing timestamp history.';
